...
const initialState = { villains: [], villain: {}, currId: 0, isLoading: false, signedin: false };
...
export const actionCreators = {
    ...
    requestVillain: (id) => async (dispatch, getState) => {
        dispatch({ type: requestVillainType });

        const url = `api/Villains/GetVillain/${id}`;
        const response = await fetch(url);
    ...
    updateVillain: (villain) => async (dispatch, getState) => {
        ...
        const data = JSON.stringify(
            { id: villain.id, name: villain.name, powers: villain.powers, hobbies: villain.hobbies }
        );
    ...
    deleteVillain: (villain) => async (dispatch, getState) => {
        ...
        const fetchTask = fetch(baseURL + "/" + villain.id, {
            method: "DELETE",
            headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
    ...
    signin: (id) => async (dispatch, getState) => {
        ...
        dispatch({ type: receiveSigninType, villain });
    }
};
